...
int main(int argc, char *argv[])
{
    ...
    if(SDL_Init(SDL_INIT_VIDEO | SDL_INIT_AUDIO | SDL_INIT_TIMER)) {
     ...
     if(avformat_open_input(&pFormatCtx, argv[1], NULL, NULL)!=0){
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Failed to open video fill!");
     ...
     if(audioStream == -1){
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Didn't find an audio stre    am");
        goto __FAIL; // Didn't find an audio stream
     }
